...
2. **同名即冲突**：名称相同（trim + NOCASE）就判冲突，包括同业务键重复导出与
   清除数据后重建的同名对象。重复导出也弹合并/覆盖窗，让用户在「恢复历史歌曲」
   与「仅保存当前内容」之间选择，避免静默覆盖丢失历史。
   - 同业务键重复导出时冲突文件就是自身旧文件，净增 0（删旧 + 写新）；
   - 无同名文件时净增 1；多个同名时净增 = 1 - 冲突数。
3. `overLimit = projected > MAX_EXPORT_COUNT`。
